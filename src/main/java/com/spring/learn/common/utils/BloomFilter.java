 ...
 *
 * @param <E> E指定了要插入过滤器中的元素的类型，eg，String Integer
...
public class BloomFilter<E> implements Serializable {
     ...
     *
     * @param bitSetSize                     指定了过滤器所需位的大小
     * @param expectedNumberOfFilterElements 指定了过滤器所能添加的元素的最大数量
     *                                       to contain.
     * @param actualNumberOfFilterElements   指定了原来过滤器的数据的数量
     *                                       <code>filterData</code> BitSet.
     * @param filterData                     原有过滤器中的BitSet对象
     */
     ...
     * 根据字符串内容生成摘要
     *
     * @param val 指定了输入的字符串。默认的编码为 UTF-8
     * @return 输出为一个long类型
     */
     ...
     *
     * @param numberOfElements 插入的元素的个数.
     * @return 误报率.
     */
     ...
     *
     * @param bit   要设置的位置.
     * @param value true表示已经成功设置。false表示改为被清除。
     */
    ...
    public static void main(String[] args) {
        BloomFilter bloomFilter = new BloomFilter<String>(8, 10000000, 5);
        bloomFilter.add("123123");
        //1561702408643
        //1561702422561
        System.out.println(bloomFilter.contains("12312"));
        //取得指定时区的时间
        System.out.println(System.currentTimeMillis());
        TimeZone zone = TimeZone.getTimeZone("GMT - 4:00");
